-- Grille planning : tout compte avec un profil voit tous les créneaux (pas seulement son « travail perso »).
-- Évite les chevauchements invisibles pour les autres élèves et aligne l’affichage sur l’agenda Google général.
...
comment on function public.planning_events_in_range(timestamptz, timestamptz) is
    'Tous les créneaux sur la plage pour tout utilisateur authentifié ayant un profil (grille partagée) + miroirs Google + e-mails inscrits cours.';
