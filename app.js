...
async function login() {
    ...
    try {
        ...
        alert("Erreur serveur. Vérifiez l'URL du script.");
        btn.disabled = false; btn.innerText = "Se connecter";
    }
}
...
function prepareReservation() {
    ...
    hSels.forEach(s => {
        s.innerHTML = "";
        for(let i=8; i<=22; i++) s.innerHTML += `<option value="${i<10?'0'+i:i}">${i}h</option>`;
    });
...
async function sendReservation() {
    ...
    const start = `${day}T${document.getElementById('startH').value}:${document.getElementById('startM').value}:00`;
    const end = `${day}T${document.getElementById('endH').value}:${document.getElementById('endM').value}:00`;

    if(new Date(end) <= new Date(start)) return alert("L'heure de fin doit être après le début.");
    ...
    const params = new URLSearchParams({
        ...
        title: title, start: new Date(start).toISOString(), end: new Date(end).toISOString()
    });
...
function showMyEvents() {
    ...
    .then(r => r.json()).then(data => {
        ...
        else listDiv.innerHTML = data.events.map(ev => `<div class="event-item"><span>${ev.start}</span> <button onclick="deleteEvent('${ev.id}')">Suppr.</button></div>`).join('');
    });
}
