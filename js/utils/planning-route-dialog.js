...
export function openPlanningRouteDialog(dialogId, ariaLabel, backLabel) {
    ...
    if (!bar) {
        ...
        bar.className =
            'planning-route-page__bar flex items-center justify-start shrink-0 bg-white px-3 py-2 max-sm:pt-[max(0.35rem,env(safe-area-inset-top,0px))]';
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className =
            'planning-route-page__back text-left text-sm font-semibold text-slate-700 hover:text-slate-900 hover:underline py-1 px-0 min-h-0 h-auto border-0 bg-transparent cursor-pointer max-w-[min(100%,28rem)] truncate';
